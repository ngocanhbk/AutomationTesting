...
public class TestCaseExecutor {
	 ...
	 *
	 * @param type
	 *            Kiểu primitive type or 1 object var.
	 *
	 ...
	 * 
	 * @param action
	 *            The method invocation action được thực hiện
	 * @param values
	 *            tham số thực tế cho invocation.
	 */
		...
		try {
			...
			for (int j = 0; j < params.length; j++) {
				if (!values[j].contains(" ")) {
					actualPrimitiveParams[j] = mapValueToObject(values[j]);
				} else {
					checkArray = true;
			...
			if (method != null) {
				if (checkArray == false) {
					method.invoke(obj, actualPrimitiveParams);
					expectResult = String.valueOf(method.invoke(obj, actualPrimitiveParams));
				} else if (checkArray == true && (params.length == 1)) {
					if (params[0].getCanonicalName().equals("int[]")) {
						int[] ints = new int[actualOneArrayParams.length];
						for (int i = 0; i < actualOneArrayParams.length; i++) {
							ints[i] = (Integer) actualOneArrayParams[i];
						}
						method.invoke(obj, ints);
						expectResult = method.invoke(obj, ints).toString();
					} else if (params[0].getCanonicalName().equals("java.lang.String[]")) {
						java.lang.String[] strings = new java.lang.String[actualOneArrayParams.length];
						for (int i = 0; i < actualOneArrayParams.length; i++) {
							strings[i] = "" + actualOneArrayParams[i];
						}
						method.invoke(obj, (Object) strings);
						expectResult = method.invoke(obj, (Object) strings).toString();
					} else if (params[0].getCanonicalName().equals("float[]")) {
						float[] floats = new float[actualOneArrayParams.length];
						for (int i = 0; i < actualOneArrayParams.length; i++) {
							floats[i] = ((Double) actualOneArrayParams[i]).floatValue();
						}
						method.invoke(obj, floats);
						expectResult = method.invoke(obj, floats).toString();
					} else if (params[0].getCanonicalName().equals("boolean[]")) {
						boolean[] booleans = new boolean[actualOneArrayParams.length];
						for (int i = 0; i < actualOneArrayParams.length; i++) {
							booleans[i] = (Boolean) actualOneArrayParams[i];
						}
						method.invoke(obj, booleans);
						expectResult = method.invoke(obj, booleans).toString();

					} else if (params[0].getCanonicalName().equals("double[]")) {
						double[] doubles = new double[actualOneArrayParams.length];
						for (int i = 0; i < actualOneArrayParams.length; i++) {
							doubles[i] = (Double) actualOneArrayParams[i];
						}
						method.invoke(obj, doubles);
						expectResult = method.invoke(obj, doubles).toString();
					} else if (params[0].getCanonicalName().equals("long[]")) {
						long[] longs = new long[actualOneArrayParams.length];
						for (int i = 0; i < actualOneArrayParams.length; i++) {
							longs[i] = (int) actualOneArrayParams[i];
						}
						method.invoke(obj, longs);
						expectResult = method.invoke(obj, longs).toString();
					}
					// xử lý thủ công cho dạng 2 tham số với tham số 1 là kiểu nguyên thủy
					// tham số là mảng
				} else if (checkArray == true && (params.length == 2)) {
					if (params[0].getCanonicalName().equals("int[]")) {
						int[] ints = new int[actualOneArrayParams.length];
						for (int i = 0; i < actualOneArrayParams.length; i++) {
							ints[i] = (Integer) actualOneArrayParams[i];
						}
						method.invoke(obj, actualPrimitiveParams[0], ints);
						expectResult = method.invoke(obj, actualPrimitiveParams, ints).toString();
					} else if (params[0].getCanonicalName().equals("java.lang.String[]")) {
						java.lang.String[] strings = new java.lang.String[actualOneArrayParams.length];
						for (int i = 0; i < actualOneArrayParams.length; i++) {
							strings[i] = "" + actualOneArrayParams[i];
						}
						method.invoke(obj, actualPrimitiveParams[0], (Object) strings);
						expectResult = method.invoke(obj, actualPrimitiveParams, (Object) strings).toString();
					} else if (params[0].getCanonicalName().equals("float[]")) {
						float[] floats = new float[actualOneArrayParams.length];
						for (int i = 0; i < actualOneArrayParams.length; i++) {
							floats[i] = ((Double) actualOneArrayParams[i]).floatValue();
						}
						method.invoke(obj, actualPrimitiveParams[0], floats);
						expectResult = method.invoke(obj, actualPrimitiveParams, floats).toString();
					} else if (params[0].getCanonicalName().equals("boolean[]")) {
						boolean[] booleans = new boolean[actualOneArrayParams.length];
						for (int i = 0; i < actualOneArrayParams.length; i++) {
							booleans[i] = (Boolean) actualOneArrayParams[i];
						}
						method.invoke(obj, actualPrimitiveParams[0], booleans);
						expectResult = method.invoke(obj, actualPrimitiveParams[0], booleans).toString();
					} else if (params[0].getCanonicalName().equals("double[]")) {
						double[] doubles = new double[actualOneArrayParams.length];
						for (int i = 0; i < actualOneArrayParams.length; i++) {
							doubles[i] = (Double) actualOneArrayParams[i];
						}
						method.invoke(obj, actualPrimitiveParams[0], doubles);
						expectResult = method.invoke(obj, actualPrimitiveParams[0], doubles).toString();
					} else if (params[0].getCanonicalName().equals("long[]")) {
						long[] longs = new long[actualOneArrayParams.length];
						for (int i = 0; i < actualOneArrayParams.length; i++) {
							longs[i] = (int) actualOneArrayParams[i];
						}
						method.invoke(obj, actualPrimitiveParams[0], longs);
						expectResult = method.invoke(obj, actualPrimitiveParams[0], longs).toString();
					}
				}
			}
		} catch (SecurityException e) {
	 ...
	 *            Class đang được thực tiện test
	 * @param chrom
	 *            chromosome sẽ test
	 */
