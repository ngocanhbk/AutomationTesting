...
public class ChromosomeFormer {
	 ...
	 * 
	 * @param chrom
	 *            chromosome đang được xét thực thi
	 * @throws IOException
	 */

	public void caculateApproadLevel(Set branchTarget) throws IOException {
		...
		for (int i = 0; i < coverBranch.toString().length(); i++) {
			Matcher matcher = pattern.matcher(coverBranch.toString().charAt(i) + "");
			if (matcher.matches()) {
				temp += coverBranch.toString().charAt(i) + "";
			} else {
				if (temp.equals("") == true) {
					continue;
				} else {
					if (temp.equals("") == false) {
						int t = 0;
						if (branchTarget.toString().contains("[" + temp + ","))
							t = 1;
						if (branchTarget.toString().contains(" " + temp + ","))
							t = 2;
						if (branchTarget.toString().contains(" " + temp + "]"))
	 ...
	 *
	 * @param sign:
	 *            MethodSignature The constructor được add
	 */
	 ...
	 *            String class đi kèm
	 * @param sign:
	 *            MethodSignature The method được add
	 */
	 ...
	 *
	 * @param type
	 *            ví dụ như "int", "String", "boolean", etc..
	 */
	 ...
	 *
	 * @param type
	 *            "float" hoặc "double", với phạm vi "[l;u]"
	 */
	 ...
	 *
	 * @param className
	 *            Name of the constructor to prepend
	 */
	 ...
	 *
	 * @param usedClass
	 *            Tập hợp tất cả các lớp được sử dụng trong chữ ký.
	 */
