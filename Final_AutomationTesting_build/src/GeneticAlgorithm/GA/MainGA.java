...
public class MainGA {
	...
	public static void main(String[] args) throws InterruptedException, IOException {
		inputs = Arrays.copyOf(args,args.length);
		// tạo testcase
		generateTestcaseWithGA(args[0]);
	...
	public static void generateTestcaseWithGA(String classUnderTest) throws IOException {
		...
		TestGenerator.pathsFile = Common.relativePath + classUnderTest + ".path";
		TestGenerator.targetFile = Common.relativePath + classUnderTest + ".tgt";
		...
		String signFile = Common.relativePath + classUnderTest + ".sign";
			...
			for (int i = idBranch[0]; i <= idBranch[idBranchString.length - 1]; i++) {
				...
				if (Population.getExtendTarget().size() > 0) {
					for (int j = 0; j < Population.getExtendTarget().size(); j++) {
						...
						// tối đa
						while (initPop.getFittest() < curFittestTarget && generationCount < Common.maxLoop) {
						...
						if (generationCount < Common.maxLoop) {
	 ...
	 * 
	 * @param listTarget
	 *            list các target của các method
	...
	public static List<String> generateExtendTarget(String subClassUnderTest, Collection subTrace) throws IOException {
		...
		// Lấy ra các branchTarget
		TestGenerator.pathsFile = Common.relativePath + subClassUnderTest + ".path";
		TestGenerator.targetFile = Common.relativePath + subClassUnderTest + ".tgt";
